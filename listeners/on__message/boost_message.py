...
class BoostMessage(commands.Cog):
    ...
    async def on_message(self, msg: discord.Message):
        if msg.type == discord.MessageType.premium_guild_subscription:
            ...
            # 15/05/2026 (EU) snow2code: A small thing..
            #  wait a bit before doing a boost message, so there isn't many of the bot's boost image
            time.sleep(3)
            if boosts_channel.last_message != self.bot.user.id:
